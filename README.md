...
- `docker run -p 8080:80 -v mysite_data:/usr/share/nginx/html` 과 같이, 포트 매핑 규칙과 볼륨 이름을 독립된 터미널 인스럭션 텍스트로 README 및 스크립트로 명시적으로 작성(Hard-coding)하여 기록했습니다.
- 이를 통해 해당 코드와 명령어 줄만 복사하면 다른 팀원이나 컨테이너 오케스트레이션 환경(CI/CD)에서도 환경 차이 없이 언제나 동일하게 네트워크 포트와 영구 스토리지 볼륨이 재현되도록 하였습니다.
